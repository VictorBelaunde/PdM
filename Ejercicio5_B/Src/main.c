...
/* Private macro -------------------------------------------------------------*/
#define LED2_TOGGLE_TIME_1				100
#define LED2_TOGGLE_TIME_2				500
...
int main(void)
{
	...
	tick_t durationLed = LED2_TOGGLE_TIME_1;
	...
	while (1) {
		...
		if (readKey_descendente()){
			if (durationLed == LED2_TOGGLE_TIME_1){
				durationLed = LED2_TOGGLE_TIME_2;
				uartSendString((uint8_t *) "Boton presionado\n\r");

			}
			else{
				durationLed = LED2_TOGGLE_TIME_1;
			}

			delayWrite(&delay_led, durationLed);
		}

		if (readKey_ascendente){
